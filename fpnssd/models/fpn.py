...
def resnet_by_layer(resnet_layer, pretrained=False, frozen=False):
...
class ResNetFPN(nn.Module):
    def __init__(self, n_layer, pretrained=False, frozen=False):
        super().__init__()
        self.encoder = resnet_by_layer(n_layer, pretrained, frozen)
...
def test():
    net = ResNetFPN(n_layer=50, pretrained=True, frozen=False)
    fms = net(torch.randn(1, 3, 128, 128))
    for fm in fms:
        print(fm.size())
